...
vector<F> prove_aggregation(aggregation_witness data, int level){
	...
	if(data.merkle_proof.size()/(2*(arity+1)) != 0){
		...
		//Transcript.push_back(prove_sha256(gkr_input, generate_randomness(10,F(312)),data.merkle_proof.size()/(2*(arity+1))));
		...
		for(int i = 0; i < arity+1; i++){
			Transcript.push_back(_P);
			//Opening_proof.push_back(Transcript[Transcript.size()-1]);
		}
		gkr_input.clear();
...
int main(int argc, char *argv[]){
	...
	/*
	vector<F> t(1ULL<<(atoi(argv[1])));
	
	test(t, generate_randomness(19,F(1)), t.size());
	printf("OK\n");
	test(t, generate_randomness(19,F(1)), t.size());
...
*/
	...
	//check_dataset(Batch,  input_dim);
		...
		clear_tensor3(net.Filters);
		clear_tensor3(net.Rotated_Filters);
		clear_tensor1(net.Weights);
		clear_tensor3(net.der);		
		
		vector<avg_layer_backprop>().swap(net.avg_backprop);
		net.avg_backprop.clear();
		
		vector<avg_layer>().swap(net.avg_layers);
		net.avg_layers.clear();
		//net.convolutions.clear();
		//net.convolutions = vector<convolution_layer>();
		for(int i = 0; i < net.convolutions.size(); i++){
			clear_matrix(net.convolutions[i].X);
			clear_matrix(net.convolutions[i].fft_X);
			clear_matrix(net.convolutions[i].fft_W);
			clear_matrix(net.convolutions[i].Prod);
			clear_matrix(net.convolutions[i].U);
			
			clear_matrix(net.convolutions[i].Out);
			clear_matrix(net.convolutions[i].Sum);
		
			clear_matrix(net.convolutions[i].Remainder);
		}
		
		//net.convolutions_backprop.clear();
		//net.convolutions_backprop = vector<convolution_layer_backprop>();
		for(int i = 0; i < net.convolutions_backprop.size(); i++){
			clear_tensor2(net.convolutions_backprop[i].der_prev);
			clear_matrix(net.convolutions_backprop[i].fft_X);
			clear_matrix(net.convolutions_backprop[i].derr);
			clear_matrix(net.convolutions_backprop[i].fft_der);
			clear_matrix(net.convolutions_backprop[i].Prod);
			clear_matrix(net.convolutions_backprop[i].Rotated_W);
			clear_matrix(net.convolutions_backprop[i].pad_der);
			clear_matrix(net.convolutions_backprop[i].fft_pad_der);
			clear_matrix(net.convolutions_backprop[i].rot_W);
			clear_matrix(net.convolutions_backprop[i].fft_rot_W);
			clear_matrix(net.convolutions_backprop[i].Prod_dx);
			clear_matrix(net.convolutions_backprop[i].U_dx);
			clear_matrix(net.convolutions_backprop[i].dx);
			clear_matrix(net.convolutions_backprop[i].U_dx_shifted);
			clear_matrix(net.convolutions_backprop[i].U_dx_remainders);
			clear_matrix(net.convolutions_backprop[i].dw);
			clear_matrix(net.convolutions_backprop[i].dw_remainders);
			clear_matrix(net.convolutions_backprop[i].Reduced_fft_dw);
			clear_tensor2(net.convolutions_backprop[i].padding);			
	}
		//net.relus.clear();
		//net.relus = vector<relu_layer>();
		for(int i = 0; i < net.relus.size(); i++){
			clear_arr(net.relus[i].input);
			clear_arr(net.relus[i].new_input);
			clear_arr(net.relus[i].output);
			clear_arr(net.relus[i].temp_output);
			clear_arr(net.relus[i].most_significant_bits);
			
		}
	
		for(int i = 0; i < net.fully_connected.size(); i++){
			clear_matrix(net.fully_connected[i].Z_new);
			clear_matrix(net.fully_connected[i].Z_prev);
			clear_matrix(net.fully_connected[i].W);
			clear_matrix(net.fully_connected[i].Remainders);			
			clear_matrix(net.fully_connected[i].Z_temp);
		}
	
		for(int i = 0; i < net.fully_connected_backprop.size(); i++){
			clear_matrix(net.fully_connected_backprop[i].dw);
			clear_matrix(net.fully_connected_backprop[i].dw_remainders);
			clear_matrix(net.fully_connected_backprop[i].Z);
			clear_matrix(net.fully_connected_backprop[i].W);

			clear_matrix(net.fully_connected_backprop[i].dx_input);
			clear_matrix(net.fully_connected_backprop[i].dx_remainders);

			clear_matrix(net.fully_connected_backprop[i].dw_remainders);
		}
		
		
		vector<relu_layer_backprop>().swap(net.relus_backprop );
		...
		vector<proof> Pogd_proof = Transcript;
		Transcript.clear();
		vector<F> proof_witness = prove_aggr(witness_matrixes,comm_aggr); 
		vector<proof> aggr_proofs = Transcript;
		Transcript = Pogd_proof;
	   ...
	   //printf("Aggregation time  : %lf, %d\n",aggregation_time, proof_witness.size());
		...
		vector<proof> Total_Transcript = Transcript;
		vector<F> Total_POGD_hashes = POGD_hashses;
		
		u_proof_temp.push_back(verify_proof(Total_Transcript));
	   	vector<proof> temp_P = mimc_sumcheck(Total_POGD_hashes);
		...
		for(int i = 0; i < arity-1; i++){
			Total_Transcript.insert(Total_Transcript.end(), Pogd_proof.begin(),Pogd_proof.end());
		}
		
		aggregation_time_recursion += (arity+1)*proving_time;
		...
		for(int i = 0; i < arity-1; i++){
			Total_POGD_hashes.insert(Total_POGD_hashes.end(),POGD_hashses.begin(),POGD_hashses.end());
		}
		u_proof.insert(u_proof.end(),Total_Transcript.begin(),Total_Transcript.end());
		u_proof.insert(u_proof.end(),aggr_proofs.begin(),aggr_proofs.end());
		...
		verify_proof(u_proof);
		
		vector<proof> pr;
   	   	
		vector<F> total_input;
		proof_hashes.insert(proof_hashes.end(),Total_POGD_hashes.begin(),Total_POGD_hashes.end());
